...
JOBNAME=${SLURM_JOB_ID}_${INPUT}_${USRNAME}
export SBATCH_JOB_NAME=$JOBNAME
...
if [[ -f $JOBNAME.sta ]]  || [[ -f Res_$JOBNAME.sta ]]; then
	...
	cat $JOBNAME.sta | gawk -v freq=$FREQ '{if($3 !~/U/){print "*Restart, read, step="$1",inc="$2", write, overlay, frequency="freq}}' | tail -1 >> Res_$JOBNAME.inp
	 $ABQDIR/abq${ABQVER} job=Res_$JOBNAME input=Res_$JOBNAME.inp oldjob=$JOBNAME cpus=$SLURM_NTASKS -verbose 3 standard_parallel=all mp_mode=mpi interactive
else
	echo "3st condition"
