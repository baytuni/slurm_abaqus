...
class LicenseManager:
    ...
    def remove_jobs(self,job_id):
        if job_id in self.running_job_ids:
            ...
            self.running_partition_dicts.pop(ind)
            self.running_partition_prios.pop(ind)
            ...
            if job_id in self.borrower_jobs:
                ...
                for i in range(len(suspended_jobs.job_ids)):
                    ...
                    answer = self.reserve_tokens(runnable_job_id, 
                                                        runnable_job_tokens)
                    ...
                    self.running_partition_dicts.append(
                                        suspended_jobs.partition_dicts[i])
    ...
    def add_jobs(self, job_id, tokens, partition_prio, partition_dict,
                   start_time):
        self.running_tokens_per_job.append(tokens)
        self.running_partition_prios.append(partition_prio)
        self.running_partition_dicts.append(partition_dict)
    ...
    def grant_tokens(self, job_id):
        ...
        partition_dict = self.read_partition(partition_name)
        partition_prio = int(partition_dict['PriorityTier'])
        ...
        if tokens <= available_tokens:            
            self.add_jobs(job_id, tokens, partition_prio, 
                          partition_dict, start_time) 
            return self.reserve_tokens(job_id, tokens), tokens
        else:
            return self.borrow_tokens(job_id, tokens, partition_prio, 
                                      partition_dict, start_time), tokens

    def borrow_tokens(self, job_id, tokens_needed, partition_prio, 
                      partition_dict, start_time):
        ...
        if releasable_tokens >= tokens_needed:
            ...
            for j in release_inds:
                ...
                popped_partition_dict = self.running_partition_dicts.pop(j)
                if popped_partition_dict['PreemptMode'].lower() == 'suspend':
                    answer = self.release_tokens(suspended_job_id, popped_tokens) 
                ...
                suspended_jobs.partition_dicts.append(popped_partition_dict)
            ...
            self.add_jobs(job_id, tokens_needed, partition_prio, 
                          partition_dict, start_time) 
            return self.SUCCESS
        else:
            return self.NOTENOUGHTOKENS

    def update_node_tokens(self, nodename, tokens):
        license_line = (LicenseConstants.ABAQUS_GRES_STRING +
                        f':{tokens}')
        cmd_to_run = ['scontrol', 'update', f'nodename={nodename}',
                       f'{license_line}']
        cmd_process = subprocess.Popen(cmd_to_run, stdout=subprocess.PIPE)
    ...
    def release_tokens(self, job_id, tokens):
    ...
    def reserve_tokens(self, job_id, tokens):
        license_line = (LicenseConstants.SACCT_ABAQUS_LICENSE_SERVER_STRING +
                        f':{tokens}')
        cmd_to_run = ['scontrol', 'update', f'jobid={job_id}',
                       f'Licenses={license_line}']
